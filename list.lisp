...
(defun delete (value list &key (test #'eql))
  (declare (type caslist list)
           (type function test))
  (declare (optimize speed))
  (let ((tail (tail list)))
    (loop (multiple-value-bind (right left) (search-cons value list test)
            (when (or (eq right tail) (not (funcall test value (car* right))))
              (return list))
            (let ((next (cdr* right)))
              (when (and (= 1 (valid right))
                         (cas (valid right) 1 0))
                (unless (cas (cdr* left) right next)
                  (search-cons (car* right) list test))
                (return list)))))))

(defun member (value list &key (test #'eql))
  (declare (type caslist list)
           (type function test))
  (declare (optimize speed))
  (let ((right (search-cons value list test)))
    (and (not (eq right (tail list)))
         (funcall test (car* right) value))))

(defun search-cons (value list test)
  (declare (type caslist list)
           (type function test))
    ...
    (loop (let* ((cons (head list))
            ...
            (loop do (when (= 1 (valid cons))
                  ...
                  while (or (= 0 (valid cons))
                            (not (funcall test (car* cons) value))))
            (setf right cons)
